        ...
        for i in range(len(locs_R)):
            ...
            if test_m > 0:
                if locs_R[i] - qrs_i[beat_C - 1] >= np.round(1.66 * test_m):  # QRS is missing
                    if int(qrs_i[beat_C - 1]) + int(np.round(0.2 * fs)) - 1 < int(locs_R[i]) - int(np.round(0.2 * fs)) + 1:
                        pks_temp = max(ecg_m[int(qrs_i[beat_C - 1]) + int(np.round(0.2 * fs)) - 1:
                                             int(locs_R[i]) - int(np.round(0.2 * fs)) + 1])
                        locs_temp = np.argmax(ecg_m[int(qrs_i[beat_C - 1]) + int(np.round(0.2 * fs)) - 1: int(locs_R[i]) - int(np.round(0.2 * fs)) + 1])
                    else:
                        pks_temp = ecg_m[int(qrs_i[beat_C - 1]) + int(np.round(0.2 * fs)) - 1]
